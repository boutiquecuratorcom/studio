...
export default function MyBrandPage() {
  ...
  const onSubmit = async (data: BrandProfileFormValues) => {
    ...
    try {
        ...
        if (data.brandColors) {
            payload.brandColors = data.brandColors.filter(color => !!color && color.match(/^#[0-9a-fA-F]{6}$/));
        }
        
        console.log('Attempting to save to Firestore path:', brandProfileRef.path);
        console.log('Payload:', payload);

        await setDoc(brandProfileRef, { ...payload, updatedAt: serverTimestamp() }, { merge: true });
